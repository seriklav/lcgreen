...
$clone = new \app\models\UserClone();
...
<div class="padd-block" id="program-daily-money">
    <div class="cycle-block">
        <?php
        /**@var $activeClone \app\models\UserClone */
        /**@var $user \app\models\User */
        $user = Yii::$app->user->identity;
        $activeClone = $user->activeClone;
        foreach ($dailyes as $daily):
            if($daily->category == \app\models\DailyMoney::CATEGORY_BONUS) {
            ...
            if($step && $daily->id == $step->step) :
                if($step->status == Step::STATUS_PROCESSING):
                    if($daily->category != \app\models\DailyMoney::CATEGORY_BONUS) :
                        $clone->step = $step->id;
                        ...
                        <div class="cycle" data-clone="<?= $daily->id ?>">
                            <div class="title">Купить клона  в площадке <?= $daily->range ?> (<?= $daily->price; ?> рублей )</div>
                        </div>
                    <?php else:?>
                        <div class="cycle">
                            <?php
...
//                            } else {
                            ...
                            //}
                            ?>
                            <div class="title">Купить место в   <?= $name ?> (<?= $daily->price; ?> рублей )</div>
                        </div>
                <?php endif; ?>
            <?php endif; ?>
        <?php endforeach; ?>
...
<div id="dialog-form" title="Вступление в цикл">
    ...
    <div class="input-block">
        ...
        <?= $form->field($model, 'sponsor')
            ->textInput(['value' => Yii::$app->user->identity->getSponsorName() ])
            ->label(false) ?>
    </div>
    <div class="input-block">
        <p>E-mail код</p>
